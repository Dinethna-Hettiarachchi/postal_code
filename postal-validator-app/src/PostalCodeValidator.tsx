...
const PostalCodeValidator = () => {
  ...
  const handleValidate = () => {
    if (input.trim()) validatePostalCode(input.trim());
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-white to-orange-50 p-8">
      <div className="max-w-6xl mx-auto">
        {/* Header */}
        <div className="bg-white rounded-lg shadow-md p-6 mb-6 border border-brown-200">
          <div className="flex items-center gap-3 mb-4">
            <Mail className="w-8 h-8 text-brown-600" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-lg shadow-md p-6 border border-brown-200">
            <h2 className="text-xl font-bold text-brown-800 mb-4">Input Postal Code</h2>
            ...
            <button
              ...
              disabled={loading}
              className="w-full bg-brown-600 text-white py-2 px-4 rounded-lg hover:bg-brown-700 transition-colors font-medium disabled:opacity-50"
            >
            ...
            {/* Result Display */}
            {result && (
              <div className={`mt-6 p-4 rounded-lg ${result.accepted ? 'bg-yellow-50 border-2 border-yellow-200' : 'bg-orange-50 border-2 border-orange-200'}`}>
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className={`font-bold ${result.accepted ? 'text-yellow-800' : 'text-orange-800'}`}>
            ...
            {/* Test Cases */}
            <div className="mt-6">
              <h3 className="text-sm font-semibold text-brown-700 mb-3">Quick Test Cases:</h3>
              <div className="grid grid-cols-2 gap-2">
                {testCases.map((test) => (
                  <button
                    ...
                    className="text-left px-3 py-2 bg-white hover:bg-brown-50 rounded text-sm transition-colors border border-brown-100"
                  >
                    <div className="font-medium text-brown-800">{test.code}</div>
                    <div className="text-xs text-brown-600">{test.desc}</div>
                  </button>
                ))}
              </div>
            </div>
          ...
          {/* PDA Execution Steps */}
          <div className="bg-white rounded-lg shadow-md p-6 border border-brown-200">
            <h2 className="text-xl font-bold text-brown-800 mb-4">PDA Execution Steps</h2>
                ...
                {steps.map((step, index) => (
                  <div
                    ...
                    className={`p-3 rounded-lg border-2 ${
                      step.state === 'q_accept' ? 'bg-yellow-50 border-yellow-300' :
                      step.state === 'q_reject' ? 'bg-orange-50 border-orange-300' :
                      'bg-white border-brown-300'
                    }`}
                  >
                    <div className="flex items-start gap-2">
                      <ArrowRight className={`w-5 h-5 mt-0.5 ${
                        step.state === 'q_accept' ? 'text-yellow-600' :
                        step.state === 'q_reject' ? 'text-orange-600' :
                        'text-brown-600'
                      }`} />
                      <div className="flex-1">
                        <div className="font-mono text-sm font-semibold text-brown-800">
                          State: {step.state}
                        </div>
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mt-6 border border-brown-200">
          <h2 className="text-xl font-bold text-brown-800 mb-4">District Postal Code Reference</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {Object.entries(districtRanges).map(([district, info]: [string, any]) => (
              <div key={district} className="p-3 bg-brown-50 rounded-lg border border-brown-100">
                <div className="flex items-center gap-2 mb-1">
                  <MapPin className="w-4 h-4 text-brown-600" />
                  <span className="font-semibold text-brown-800">{district}</span>
                </div>
                <div className="text-sm text-brown-700">
                  <div>Prefix: {info.prefix}xxx</div>
                  <div>Range: {info.range.map(([min, max]: [number, number]) => `${min}-${max}`).join(', ')}</div>
                  <div className="text-xs text-brown-500 mt-1">{info.description}</div>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* Formal Definition */}
        <div className="bg-white rounded-lg shadow-md p-6 mt-6 border border-brown-200">
          <h2 className="text-xl font-bold text-brown-800 mb-4">PDA Formal Definition</h2>
          <div className="space-y-3 text-sm font-mono text-brown-700">
